...
if [ "$#" == 0 ];then
	...
	echo "Usage: ./test_installer_without_params.sh path/to/wiperdog_installer_jar"
	echo "Example : ./test_installer_without_params.sh wiperdog-0.2.5-SNAPSHOT-unix.jar"
	exit
fi
 wdInstaller=$1
 baseDir=$(pwd)
...
echo "-----------------------------------------------------------------------------------------"
echo "Case 1: Install with default input"
echo "** Clean data..."
sudo rm -rf $wdDir
if [ -d $wdDir ]; then
...
expect<<DONE
	spawn sudo java -jar $wdInstaller
...
sleep 10
echo "-----------------------------------------------------------------------------------------"
echo "Case 2: Install without params (manual input)"
echo "** Clean data..."
sudo rm -rf $wdDir
if [ -d $wdDir ]; then
	echo "Failure to clean data .."
